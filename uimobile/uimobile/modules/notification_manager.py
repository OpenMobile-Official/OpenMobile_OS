...
class Message:
    def __init__(self, text, is_ring=False):
        self.text = text
        self.alpha = 0
        self.state = "fade_in"
        self.is_ring = is_ring  # 🔔 mark if this is a ringing message
...
class NotificationManager:
    def __init__(self, screen_width, screen_height, font=None, sound_path="sounds/notif.wav"):
        ...
        if os.path.exists(sound_path):
            ...
            print(f"⚠️ Sound file not found: {sound_path}")
                # Initialize ringtone
        self.ringtone = None
        self.ringtone_path = os.path.join(os.path.dirname(__file__), "ringtone.mp3")
        if os.path.exists(self.ringtone_path):
            try:
                # Mixer might already be initialized above
                if not pygame.mixer.get_init():
                    pygame.mixer.init()
                self.ringtone = pygame.mixer.Sound(self.ringtone_path)
                print(f"🔔 Ringtone loaded: {self.ringtone_path}")
            except Exception as e:
                print(f"❌ Failed to load ringtone: {e}")
        else:
            print(f"⚠️ Ringtone file not found: {self.ringtone_path}")
    ...
    def push(self, text, duration=3):
        ...
        # Message mode
            ...
            try:
                ring_text = text.split('ring="')[1].split('"')[0]
                self.active_message = Message(ring_text)
                self.create_message_button(is_ring=True)
                if self.ringtone:
                    print("🔔 Playing ringtone...")
                    self.ringtone.play(loops=-1)  # Loop indefinitely
                else:
                    print("⚠️ No ringtone loaded.")
            except Exception as e:
                print(f"❌ Failed to parse ring message: {e}")
            return
        elif text.startswith('rdialogue='):
            try:
                yup_text = text.split('rdialogue="')[1].split('"')[0]
                action = None
                if 'if=yesrun' in text:
                    try:
                        action = text.split('if=yesrun"')[1].split('"')[0]
                    except IndexError:
                        print("❌ Failed to parse yupelf action")

                # Create a special dialogue that rings
                self.active_dialogue = Dialogue(yup_text, action)
                self.create_dialogue_buttons(is_ring=True)

                if self.ringtone:
                    print("🔔 Playing ringtone forringf dialogue...")
                    self.ringtone.play(loops=-1)
                else:
                    print("⚠️ No ringtone loaded for rind dialogue.")
            except Exception as e:
                print(f"❌ Failed to parse ring dialogue: {e}")
            return
    ...
    def create_dialogue_buttons(self, is_ring=False):
        ...
        def on_yes():
            print("✅ YES clicked")
            # Stop ringtone if this is a ringing dialogue
            if is_ring and self.ringtone:
    ...
    # MESSAGE HANDLING
    # ------------------------
    def create_message_button(self, is_ring=False):
        w, h = 100, 40
        y = self.screen_height - 80
        ...
        def on_okay():
            print("✅ OK clicked")
            # Stop ringtone if it's a ring-type message
            if is_ring and self.ringtone:
                print("🔇 Stopping ringtone...")
                self.ringtone.stop()
            self.active_message = None
